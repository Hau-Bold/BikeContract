...
/**
 * the class ContractModelAdvarics
 *
 */
public class ContractModelAdvarics extends TableModel {
	 ...
	 */
	public ContractModelAdvarics(String[] nameOfColumns) {
		super(nameOfColumns);
	}
	...
	public boolean isCellEditable(int row, int column) {
		...
		if (row != -1) {
			/**
			 * quantity,productId, ean, articel, producer aren't allowed to be editable,
			 * except the data set is not from erp data
			 */
	...
	public void appendErpDataToErpData(ErpDataSet erpDataSet, Boolean value) {
		...
		String articleNumber = erpDataSet.getArticleNumber();
		if (!lstIdentifier.contains(articleNumber)) {

			/** for reacting on scanning same article */
			lstIdentifier.add(articleNumber);
			...
			newData[getCountOfSellpositions()][7] = articleNumber;
			...
			int rowOfDataSet = getRowExistingErpDataset(articleNumber);
	...
	public void appendSellPositions(List<SellPosition> lstSellPosition) {
		...
		for (int row = 0; row < coutOfSellpositions; row++) {
			...
			String articleNumber = sellPosition.getArticleNumber();
	...
	public void removeRow(int selectedRow) {
		if (dataEntries != null) {
			...
			String productId = (String) getValueAt(selectedRow, 1);
			lstIdentifier.remove(productId);
